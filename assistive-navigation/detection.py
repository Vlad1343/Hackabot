...
class DetectionEngine:
    ...
    def _direction_for_bbox(self, bbox: tuple[int, int, int, int], width: int, mode: str) -> str:
        det_cfg = self.config["detection"]
        left_boundary = det_cfg.get(f"left_boundary_{mode}", det_cfg["left_boundary"])
        right_boundary = det_cfg.get(f"right_boundary_{mode}", det_cfg["right_boundary"])

        x1, _, x2, _ = bbox
        cx = (x1 + x2) / 2.0
        ratio = cx / float(max(1, width))
    ...
    def detect(self, frame: np.ndarray, mode: str) -> tuple[list[DetectionEvent], list[dict]]:
        ...
        try:
            ...
            for result in results:
                for box in result.boxes:
                    ...
                    bbox = (x1, y1, x2, y2)
                    direction = self._direction_for_bbox(bbox, frame_w, mode)
            ...
            for (label, direction), items in grouped.items():
                ...
                debug_detections.append(
                    {
                        "label": f"{'several_' if count > 1 else ''}{label}",
                        "direction": direction,
                        "confidence": chosen["confidence"],
                        "bbox": chosen["bbox"],
                    }
